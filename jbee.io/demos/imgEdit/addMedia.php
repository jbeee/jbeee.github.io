...
$projID = $show[1];
		$buttonTxt = $show[0];
			if($show[0]=='update')
			{
				if($show[1]=='new')
				{
					echo '<TITLE>ADD NEW ARTICLE</TITLE>';
					$idxHTML = "lots oh stuff";
					$name = trim($_POST["name"]);
					$dName = str_replace(' ', '',strtolower($name));
					if($name=='')
					{
						$createError = "<b>ADD FAILED</b>: NAME REQUIRED TO CREATE NEW ARTICLE";
					}
					else if(is_dir('../media/'.$dName))
					{
						$createError ='<b>ADD FAILED</b>: An Article named <strong>\''.$name.'\'</strong> already exists.';
					}
					else
					{
						mkdir('../media/'.$dName, 0777);
						$insert = "INSERT INTO  portfolio (5,name,keywords,location,description,size) VALUES ('";
						...
						$projID = $row[pID];
						$concatMe = "UPDATE portfolioCat SET pOrder = CONCAT(pOrder, ',".$projID."')WHERE cID=5";
						mysql_query($concatMe);
					
						$createIdx = fopen('../media/'.$dName.'/index.php', 'w') or die("can't open file");
			...
			for($i=0;$i<count($edited);$i++)
			{
				if($edited[$i] != 0)
				{
					if(($i==0)||($i==3)){						
						...
						copy($img[$i+1], '../media/'.$dName.'/'.$dName.'_'.$img[0].$tExt);
 ...
 {case 1:$cat1 ='selected';break;case 2:$cat2 ='selected';break;case 3:$cat3 ='selected';break;case 4:$cat4 ='selected';break;case 5:$cat5 ='selected';break;}
			 ...
			 $teamExt[5] = array('real_estate_agent',$row[real_estate_agent],$showTeam[5]);
			 if($row[thmb]!='%'){$thmb='../media/'.$dName.'/'.$row[thmb]; $thmbC = '<img src="'.$thmb.'">';}
			  $singleCSS = 'display:none;';	
			  $doubleR = 'checked';
			  $sideNum = '2';
			 if($row[topImg]!='%')
			 {
				$topImg='../media/'.$dName.'/'.$row[topImg];$topImgC2= '<img src="'.$topImg.'">';			 
			 ...
			 {$btmImg='../media/'.$dName.'/'.$row[btmImg]; $btmImgC = '<img src="'.$btmImg.'">';} 
			 $pOrder = $row[pOrder];
			 $allImgs=array();	 
			 $pos = explode(',',$pOrder);
...
Article:<br><input class="portBasics" name="name" size=20 value="<?php echo $name;?>"><br>
Source:<br>
...
<table><tr><td width="100px">Page Layout:</td><td>
<input type = "hidden" name="category" id="cats" value ="5">
 <span class="smallRadio">
<input type="radio" name="shPpl" class="pageNum" value="1"/>one page &nbsp;&nbsp;&nbsp;
<input type="radio" name="shPpl" class="pageNum" value="2"/>two page
</span>
</td></tr></table>
<br />
Article KeyWords:<br><textarea name="keywords" rows=3 cols=48 id="kws"><?php echo $kw;?></textarea>
